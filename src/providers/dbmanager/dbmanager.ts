...
export class DbmanagerProvider {
  ...
  deleteDatabase() {
    let sql = 'DROP TABLE points';
    return this.db.executeSql(sql, []);
  }

  // Funcion que crea la tabla points con sus atributos
  createPointTable() {
    let sql = 'CREATE TABLE IF NOT EXISTS points(id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, description TEXT, date TEXT, lat REAL, long REAL, type TEXT, latDegrees TEXT, lngDegrees TEXT, latMinutes TEXT, lngMinutes TEXT, latSeconds TEXT, lngSeconds TEXT, lngHemisphire TEXT, latHemisphire TEXT, north TEXT, east TEXT, origin TEXT)'; // Query a la DB
  ...
  createPoint(point: any){
    let sql = 'INSERT INTO points(name, description, date, lat, long, type, latDegrees, lngDegrees, latMinutes, lngMinutes, latSeconds, lngSeconds, lngHemisphire, latHemisphire, north, east, origin) VALUES(?,?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'; // Query a la DB, Asigna los valores a cada campo mediante el operador ? (Evita Ataques como SQLInjection y son buenas practicas)
    return this.db.executeSql(sql, [point.name, point.description, point.date, point.lat, point.long, point.type, point.latDegrees, point.lngDegrees, point.latMinutes, point.lngMinutes, point.latSeconds, point.lngSeconds, point.lngHemisphire, point.latHemisphire, point.north, point.east, point.origin ]);
  }
